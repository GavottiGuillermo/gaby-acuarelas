...
- El retorno de Mercado Pago al abandonar Checkout Pro puede incluir `payment_id=null`. La interfaz descarta ese valor por no ser una referencia de pago, deja de mostrar una confirmación indefinida y vuelve al carrito con un aviso de que no hubo cobro. Si una versión anterior ya eliminó esos parámetros, una recarga consulta al proveedor y limpia la espera local cuando confirma que no existe un pago asociado. La orden interna permanece pendiente porque un parámetro del navegador no puede autorizar una transición a `cancelled`; un rechazo con identificador real continúa conciliándose contra la API autenticada.
- Las preferencias Mercado Pago nuevas tienen 30 minutos de vigencia mediante los campos oficiales `expires`, `expiration_date_from` y `expiration_date_to`. El servidor inicia un conciliador junto con la aplicación y lo repite cada 5 minutos; también está disponible como `npm run payments:reconcile`.
- El conciliador consulta pagos y preferencias con el Access Token. Conserva pagos reales `pending`, `in_process` o `authorized`; aplica estados terminales con el registro idempotente existente; y cancela una orden sin pago sólo cuando la preferencia autenticada venció y pasaron 15 minutos de margen. Las preferencias históricas sin vencimiento reciben primero una fecha de cierre y se revisan nuevamente antes de cualquier cancelación.
- Los abandonos y estados terminales limpian la clave idempotente local sin borrar el carrito, por lo que un nuevo intento crea otra orden y preferencia en vez de reutilizar una abandonada. Los fallos de lote o pendientes de más de 24 horas activan una alerta segura en logs, sin cuerpos ni datos personales; el correo administrativo continúa preparado para la etapa 5.
- `npm run check` aprobó la conciliación programada con 55 pruebas automatizadas (54 aprobadas y la prueba PostgreSQL opcional omitida por no estar configurada `TEST_DATABASE_URL`), compuerta de etapa y catálogo el 2026-09-24.
